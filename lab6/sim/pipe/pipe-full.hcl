# name:JiTing
# ID:5140379049
# description of how and why modify
# First, implement the iaddl and leave instructions -> reduce the number of instructions
# Second, optimize the combination of IMRMOVL and IRMMOVL -> avoid load/use hazard
# Third, optimize the combination of IIADDL and IJLE -> avoid mispredicted branch hazard
# Changes are shown in comments containing the keyword "modified"
#/* $begin pipe-all-hcl */
####################################################################
...
int f_pc = [
	# modified
	# if IIADDL + IJLE, use E_valA and E_valB to decide f_pc
	D_icode == IJXX && E_icode == IOPL && D_ifun == 1 && E_ifun == 2 && ( E_valA <= 0 ) && ( E_valB <=0 ) : D_valC;
	D_icode == IJXX && E_icode == IOPL && D_ifun == 1 && E_ifun == 2 && ( E_valA > 0 ) && ( E_valB >0 ) : D_valP;
	# Mispredicted branch. ( except IIADDL + IJLE )  Fetch at incremented PC
	M_icode == IJXX && !M_Cnd && !( W_icode == IOPL && M_ifun == 1 && W_ifun == 2 ) : M_valA;
...
#modified
# if IMRMOVL + IRMMOVL, get e_valA from m_valM
int e_valA = [
	E_icode == IRMMOVL && M_icode == IMRMOVL : m_valM; 
...
bool F_bubble = 0;
bool F_stall =
	# Conditions for a load/use hazard ( except IMRMOVL + IRMMOVL )
	E_icode in { IMRMOVL, IPOPL } &&
	 E_dstM in { d_srcA, d_srcB } && D_icode != IRMMOVL ||
...
bool D_bubble =
	# Mispredicted branch ( except IIADDL + IJLE )
	(E_icode == IJXX && !e_Cnd && !( M_icode == IOPL && M_ifun == 2 && E_ifun == 1 ) ) ||
	# Stalling at fetch while ret passes through pipeline
...
#modified
bool E_bubble =
	# Mispredicted branch ( except IMRMOVL + IRMMOVL and IIADDL + IJLE)
	(E_icode == IJXX && !e_Cnd && !( M_icode == IOPL && M_ifun == 2 && E_ifun == 1 ) ) ||
