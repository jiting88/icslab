...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    if(val<0)
        sign=TRUE;
    if(op==A_ADD&&(((argA<0)==(argB<0))&&((val<0)!=(argA<0))))
        ovf=TRUE;
    long_t opp=~argA+1;
    if(op==A_SUB&&(((opp<0)==(argB<0))&&((val<0)!=(opp<0))))
        ovf=TRUE;
...
stat_t nexti(y86sim_t *sim)
{
    ...
    switch(icode){
        case I_RRMOVL:case I_IRMOVL:case I_MRMOVL:case I_RMMOVL:case I_ALU:case I_PUSHL:case I_POPL:
    ...
    switch (icode) {
      ...
      case I_IRMOVL: /* 3:0 F:regB imm */
            set_reg_val(sim->r,regB,immediate);
            break;

      case I_RMMOVL: /* 4:0 regA:regB imm */
            val=get_reg_val(sim->r,regA);
            if(!set_long_val(sim->m,get_reg_val(sim->r,regB)+immediate,val)){
                err_print("PC = 0x%x, Invalid instruction address",sim->pc);
                return STAT_ADR;
            }
            break;
      ...
      case I_MRMOVL: /* 5:0 regB:regA imm */
            if(!get_long_val(sim->m,get_reg_val(sim->r,regB)+immediate,&val)){
                err_print("PC = 0x%x, Invalid data address 0x%x",sim->pc,get_reg_val(sim->r,regB)+immediate);
                return STAT_ADR;
            }
      ...
      case I_ALU: /* 6:x regA:regB */
           val=compute_alu(ifun,get_reg_val(sim->r,regA),get_reg_val(sim->r,regB));
      ...
      case I_CALL: /* 8:x imm */
            esp=get_reg_val(sim->r,4);
            set_reg_val(sim->r,4,esp-4);
            if(!set_long_val(sim->m,esp-4,next_pc)){
                err_print("PC = 0x%x, Invalid stack address 0x%x",sim->pc,esp-4);
                return STAT_ADR;
            }
            next_pc=immediate;
            break;
      ...
      case I_RET: /* 9:0 */
            esp=get_reg_val(sim->r,4);
            if(!get_long_val(sim->m,esp,&retadd)){
                err_print("PC=0x%x, Invalid instruction address",sim->pc);
                return STAT_ADR;
            }
            set_reg_val(sim->r,4,esp+4);
      ...
      case I_PUSHL: /* A:0 regA:F */
            ...
            esp=get_reg_val(sim->r,4);
            set_reg_val(sim->r,4,esp-4);
            if(!set_long_val(sim->m,esp-4,val)){
                err_print("PC = 0x%x, Invalid stack address 0x%x",sim->pc,esp-4);
                return STAT_ADR;
            }
            break;
